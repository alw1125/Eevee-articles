...
<script>
  ...
  let article_id = data.article.article_id; 
  let tempImage; 
  let image = data.article.image;
  $: imageWidth = data.article.image_width;
  $: imageHeight = data.article.image_height;
...
<form on:submit|preventDefault={handlePost}>
  ...
  <ImageUpload on:upload={handleEdit} />
